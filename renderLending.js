...
// Calcola media generale Revisioni
  ...
  mkChart('smeDeliberaChart', {type:'bar', data:{labels:smeMonthsData.map(d=>d.month), datasets:[{label:'Positive', data:smeMonthsData.map(d=>d.pos), backgroundColor:PALETTE.pos}, {label:'Negative', data:smeMonthsData.map(d=>d.neg), backgroundColor:PALETTE.danger}, {label:'Media Mensile', data:Array(smeMonthsData.length).fill(smeTotalAvg), type:'line', borderColor:PALETTE.warn, borderWidth:2, fill:false, pointRadius:0, borderSkipped:false, segment:{borderDash:()=>[0]}}, {label:'Target', data:Array(smeMonthsData.length).fill(72), type:'line', borderColor:PALETTE.navy, borderWidth:2, fill:false, pointRadius:0, borderSkipped:false, segment:{borderDash:()=>[5,5]}}]}, 
